...
#
# PORT is the port the server binds. The TWG MCP Control Plane injects its own
# PORT at runtime; this ENV is just the standalone default. MCP_PORT remains a
# fallback for older compose/standalone setups (server.py: PORT > MCP_PORT).
...
# we use Python's stdlib to open a TCP socket to the listen port (PORT, then
# MCP_PORT, else 8000 — matching server.py). Exit 0 = healthy.
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD python -c "import socket,os; socket.create_connection(('127.0.0.1', int(os.environ.get('PORT') or os.environ.get('MCP_PORT') or '8000')), 3)" || exit 1
